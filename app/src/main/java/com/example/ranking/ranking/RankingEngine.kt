...
object RankingEngine {
    ...
    // ÖLÜ KOD KALDIRILDI (2026-09-02, denetim dalgası):
    // createSwissMatchesWithState / createSwissMatches / createSwissMatchesAdvanced
    // buradaydı. 1. tur dalları `half = size / 2` aritmetiğiyle tek sayıda son
    // öğeyi HİÇBİR çifte sokmuyor (maçsız/byesız/puansız), `shuffled()` yüzünden
    // düşen öğe her koşumda değişiyor ve matchNumber hiç atanmıyordu; 2+ tur
    // dalları da tekrar eşleşmeyi serbest bırakıyordu. Tek çağıran
    // (initializeSwiss'in 1. turu) SwissSystem'e taşınınca üçü de çağrısız
    // kaldı ve KALDIRILDI. Dalganın dersi: kod yorumunda "düzeltildi" yazması,
    // düzeltmenin ÇAĞRILDIĞINI göstermez — ölü ama kusurlu kod tuzaktır.
